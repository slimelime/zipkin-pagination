...
const zipkinFetch = (zipkinUrl, filters, serviceName = 'nginx') => async (
  start, end) => {
  const iterateQuery = async (start, end, acc) => {
    ...
    const veryFirst = r.reduce(r.minBy(spanTimeStamp), [{timestamp: Infinity}], traces)
    ...
    if (traces.length === pageSize && newEnd > start) {
      return iterateQuery(start, newEnd, r.uniqBy(r.pipe(r.head, r.prop('traceId')))(
        r.concat(acc, traces)))
    }
